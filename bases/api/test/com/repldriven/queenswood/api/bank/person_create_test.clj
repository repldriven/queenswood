(ns ^:eftest/synchronized com.repldriven.queenswood.api.bank.person-create-test
  "A person who already holds a membership creates another bank for
  their company: the create returns 201 with the second bank and their
  owner membership, and the command it sends names the person as the
  owner and as the actor, and binds the company (AC-11's base half).
...
(def ^:private request
  {:auth {:principal-type :user
   ...
   :parameters {:body {:company-number "SC998137" :name "Second Bank"}}
   :identity-provider
...
(deftest a-member-creates-a-second-bank-test
  (let [sent (atom nil)
    ...
    (with-redefs [companies/lookup (fn [_ company-number]
                                     {:status 200
                                      :body {:company-number company-number
                                             ...
                                             :registry-id
                                             "gb-companies-house"}})
                  SUT/send-create-bank (fn [_ data]
                                         (reset! sent data)
                                         {:status 200
                                          :body {:bank-id second-bank-id
                                                 :membership membership}})
                  banks/get-bank-view (fn [_ bank-id] {:bank-id bank-id})]
      (let [response (SUT/create-bank request)]
        (testing "returns 201 with the second bank rather than 409"
          (is (= 201 (:status response)))
          (is (= {:bank-id second-bank-id :client-secret "secret"}
                 (select-keys (:body response) [:bank-id :client-secret])))
          (is (= (assoc membership
        ...
        (testing "sends the person as the owner and as the actor"
          ...
          (is (= {:kind :actor-kind-member :principal-id user-id}
                 (:actor @sent))))
        (testing "binds the company, on the defaults a person cannot choose"
          (is (= "SC998137" (get-in @sent [:company-binding :company-number])))
          (is (= {:status :bank-status-test :tier "micro" :currencies ["GBP"]}
                 (select-keys @sent [:status :tier :currencies]))))))))
