...
(`GET /v1/banks/{bank-id}/policies`) you select from, with a per-policy
capability/limit `PolicyMatrix` below. Replaced on the `/policies` route by
the resolved **effective** view (`Policies.svelte`,
`GET /v1/banks/{bank-id}/effective-policy`), which shows one decision per
scope with provenance and no policy selector.
