(ns com.repldriven.queenswood.demo-digital-bank.interface
  ...
  `demo-digital-bank/bank` component, carrying the store, the
  ...
  The platform tells the bank through a webhook endpoint the seed
  registers. `receive` takes a delivery and answers it, and `events`
  holds a customer's stream open; a delivery that does not verify is
  an unauthorized anomaly of kind `:webhook/unsigned`, `:webhook/stale`
  or `:webhook/invalid-signature`.
  ...
  (:require
    ...
    [com.repldriven.queenswood.demo-digital-bank.core :as core]))
...
(defn receive
  "Take a delivery from the platform. Verifies the Standard Webhooks
  headers under the bank's secret, records the notification under the
  platform's id, and answers `{:notification-id :status}` — `accepted`
  where there is work to do on it, `done` where it was already
  resolved — before reading the record back, writing it against the
  customer it belongs to and pushing it on their stream.

  Args:
  - bank: the started bank component.
  - headers: the request headers, lower-cased; `webhook-id`,
    `webhook-timestamp` and `webhook-signature` are read.
  - body: the request body as delivered, as bytes."
  [bank headers body]
  (core/receive bank headers body))

(defn events
  "Hold the customer's event stream open, calling `emit` with each
